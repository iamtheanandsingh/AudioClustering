...
def load_and_process_audio(file_path, frame_size_ms=60, overlap=0.5, n_mels=40, n_mfcc=30):
    y, sr = librosa.load(file_path, sr=48000)
    frame_size = int(sr * frame_size_ms / 1000)
    hop_length = int(frame_size * (1 - overlap))
    n_fft = frame_size  # Setting n_fft equal to frame size
...
def smooth_labels(predicted_labels, window=7):
...
def run_art2_clustering(features, true_labels, vigilance_values, max_clusters):
    ...
    for vigilance in vigilance_values:
        art2 = ART2(num_features=features.shape[1], vigilance=vigilance, learning_rate=0.1, max_epochs=15, max_clusters=max_clusters)
        art2.train(features)
...
def evaluate_clustering(time_series_data, vigilance_values, max_clusters):
    results = []
    for idx, series_df in enumerate(time_series_data):
        ...
        smoothed_labels = run_art2_clustering(features, true_labels, vigilance_values, max_clusters)
        accuracy = accuracy_score(true_labels, smoothed_labels)
...
def plot_discrete_gantt(clustering_results, save_path="gantt_chart"):
    for result in clustering_results:
        ...
        mapped_labels = smoothed_labels  # Since labels are already mapped

        # Create a figure with two subplots side by side
        fig, axes = plt.subplots(1, 2, figsize=(20, 5), sharey=True)

        # First subplot: Predicted Class MFCCs vs Time
        axes[0].step(timestamps, smoothed_labels, where='post', label="Predicted Class MFCCs", color="blue", alpha=0.8)
        axes[0].set_xlabel("Time")
        axes[0].set_ylabel("Class")
        axes[0].set_title(f"Predicted Class MFCCs vs Time (Series {result['time_series']})")
        axes[0].legend()

        # Second subplot: Mapped Class ID vs Time
        axes[1].step(timestamps, true_labels, where='post', label="Mapped Class ID", color="black", alpha=0.6)
        axes[1].set_xlabel("Time")
        axes[1].set_title(f"Mapped Class ID vs Time (Series {result['time_series']})")
        axes[1].legend()
...
clustering_results = evaluate_clustering(time_series_data, vigilance_values=[0.5, 0.6, 0.7], max_clusters=5)
